...
import mozilla.components.service.fxa.manager.ext.withConstellationIfPossible
...
internal class DefaultController(
    ...
    override fun refreshSyncedTabs() {
        scope.launch {
            accountManager.withConstellationIfPossible {
    ...
    override fun syncAccount() {
        view.startLoading()
        scope.launch {
            accountManager.withConstellationIfPossible { refreshDevices() }
